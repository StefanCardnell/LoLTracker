...
//2 CHANGES NEED TO BE MADE TO PERSONALISE THIS AUTOMATED CODE. LOOK BELOW.
...
int main(){
    ...
    string server = "na", platformid = "NA1", name = "theoddone", nameid; //(CHANGE 1 OF 2): CHANGE SERVER, PLATFORMID, NAME (NOT NAMEID) TO THE DESIRED VALUES
    ...
    while(true){
        ...
        string url;
        string key = "?api_key="; //(CHANGE 2 OF 2): APPEND YOUR API KEY TO THIS
        ...
        if(nameid == ""){
			...
			erase();
			string error = "No such players exists on this server. Exiting...";
        ...
        if(!data.empty()){
            ...
            if(patternandrecord("\"gameId\":", position, ',', gameid)){
                ...
                erase();
		string error = name + " is not in a game.";
		mvprintw(row/2 - 1, (col-error.size())/2, error.c_str());
		refresh();
                sleep(60);
                continue;
